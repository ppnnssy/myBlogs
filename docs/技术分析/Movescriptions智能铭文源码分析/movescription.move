module smartinscription::movescription {
    ...
    fun init(otw: MOVESCRIPTION, ctx: &mut TxContext) {
        ...
        // 创建一个TickRecord，然后记录到deploy_record里了（就是MOVE，所以MOVE是合约部署时就创建的tick）
    ...
    // 消耗“MOVE” Movescription，部署新的Tick 需要提供和Movescription对应的TickRecord
    ...
    public fun do_mint(
        ...
        // 如果current_epoch已被epoch_records记录
        ...
        // 如果没有记录
          ...
          // 创建一个新的EpochRecord，编号就是current_epoch
    ...
    // 修改EpochRecord
    // 已存在的地址加在原mint_fees上，没有的新建mint_fees
    ...
    // 生成一个新的EpochRecord，调用者默认在players中
    ...
    fun settlement(tick_record: &mut TickRecord, epoch: u64, settle_user: address, now_ms: u64, ctx: &mut TxContext) {
        ...
        // 所有剩余未发的铭文不够两个epoch，就会合并的一个epoch
        ...
        while (idx < players_len) {
            ...
            // 把fee_balance从用于记录的EpochRecord中取出，放进铭文Movescription中
        ...
        let real_epoch_amount = per_player_amount * players_len; // 这个epoch实际发放数目
        // 如果实际发放的比较少，剩余的送给本epoch最后一个player做为奖励
    ...
    // 从inscription中分割出一个新的Movescription
    public fun do_split(
